Create a single horizontal sprite strip for the Codex app digital pet `emberbite` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: tiny dragon pet, red or black scales, small wings, glowing eyes, playful but slightly mischievous, breathes tiny fire when tasks run, sleeps when idle.
